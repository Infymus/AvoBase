...
unit MasterData_ReportExpenseListByCycleUnit;
...
type
   tMasterDataReportExpenseListByCycle = class(tQuery)
   private
   	fQuery : tQuery;
...
constructor tMasterDataReportExpenseListByCycle.Create( inMasterData : tMasterData; inOrgID : string; inStartYear, inEndYear, InStartNum, InEndNum : integer );
var
...
begin
   ...
   sqlText := 'SELECT E.*, C.ID, C.NUM, C.CYEAR, C.ORG_ID ' +
      ' FROM ' + masterData.GetTable_Expense_List + ' E' +
...
destructor tMasterDataReportExpenseListByCycle.destroy;
begin
	FreeAndNil(fQuery);
   inherited;
end;
...
procedure tMasterDataReportExpenseListByCycle.HandleCalculated(DataSet: TDataSet);
var
   tempStr : String;
begin
