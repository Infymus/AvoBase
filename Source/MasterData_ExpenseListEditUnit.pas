...
unit MasterData_ExpenseListEditUnit;
...
type
   tMasterDataExpenseListEdit = class(tQuery)
   private
   	fQuery : tQuery;
      fID : string;
...
constructor tMasterDataExpenseListEdit.Create(inMasterData: tMasterData; inID : string);
var
...
begin
   ...
   sql := 'SELECT * FROM ' + fMasterData.GetTable_Expense +
...
destructor tMasterDataExpenseListEdit.destroy;
begin
	FreeAndNil(fQuery);
   inherited;
end;
...
function tMasterDataExpenseListEdit.fGetTotAmount: currency;
var
   totQuery : tQuery;
begin
   totQuery := fMasterData.GetQuery();
   totQuery.SQL.Text := 'SELECT E_ID, SUM(AMOUNT) AS TOTAMT FROM ' + fmasterdata.GetTable_Expense +
      ' WHERE E_ID = ' + masterData.WrapDBID( fID ) +
      ' GROUP BY E_ID';
   totQuery.Open();
...
procedure tMasterDataExpenseListEdit.HandleCalculated(DataSet: TDataSet);
begin
   DataSet.FieldByName('ORGNAME').Value := Org_GetOrgNameByOrgID( self.FieldByname('ORG_ID').AsString );
   DataSet.FieldByName('EXPTYPE').Value := Expense_GetExpenseTypeNameByID( self.FieldByname('ET_ID').AsString )
   //
end;

procedure tMasterDataExpenseListEdit.Update(inOrgID: string);
var
   sql : string;
begin
	self.Close();
...
procedure tMasterDataExpenseListEdit.Update();
var
   sql : string;
begin
	self.Close();
