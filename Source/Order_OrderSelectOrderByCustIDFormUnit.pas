 (*
 * AvoBase - Point of Sale Management Software.
 * (c) 2011-2026 by Infymus.
 * Designed for CodeGear Embarcadero Delphi 2009 Architect.
 * Uses the Borland Database Engine (BDE) utilizing Paradox.
 * Will compile and run under Windows 10 or 11.
 *
 *)

unit Order_OrderSelectOrderByCustIDFormUnit;

{ This form requires a Customer ID and an Order Status Type }

interface uses
   masterdataunit,
   constantsunit,
   toolboxunit,
   img_storageformunit,
   actionunit,
   avobase_baseform_menuunit,
   masterdata_BaseDataClassUnit,
   masterdata_basegridunit,
   masterdata_navigationtoolunit,
   AvoBase_ToolBarUnit,
   AvoBase_BaseForm_SelectUnit,
   MasterData_SelectOrderByCustIDUnit,
   toolbox_customertoolboxunit,
   //
   windows,
   messages,
   sysutils,
   variants,
   classes,
   ActnList,
   graphics,
   controls,
   forms,
   dialogs,
   stdctrls,
   extctrls,
   ComCtrls,
   ToolWin,
   Mask,
   DB,
   jpeg;

type
   tOrderSelectOrderByCustIDForm = class(TAvoBase_BaseForm_Select)
   private
      fCustID : string;
      fOrdStat : tOrderStatusTypes;
      fLoadOrderEvent : tLoadOrderEvent;
      OrderDetailListQuery : tMasterDataOrderListByCustID;
      function fOrderID : string;
   	procedure HandleActionListUpdate(Action: TBasicAction; var Handled: Boolean);
      procedure HandleActionExecute(sender : tObject; actionID : integer);
      procedure HandleDoubleClick( sender : tObject );
   	procedure HandleQueryUpdate(Sender: TObject; Field: TField);
   public
      procedure StartUpForm();
      procedure UpdateQuery();
      procedure StatBarUpdate();
      property OnLoadOrderEvent : tLoadOrderEvent read fLoadOrderEvent write fLoadOrderEvent;
      property OrderID : string read fOrderID;
      constructor Create( owner: TComponent; inCustID : string; OrdStat: tOrderStatusTypes; InCaption : string; isTopBarVisble : boolean); overload;
   end;

implementation

{$R *.dfm}

//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//

constructor tOrderSelectOrderByCustIDForm.Create(owner: TComponent; inCustID : string; OrdStat: tOrderStatusTypes; InCaption: string; isTopBarVisble: boolean);
begin
	inherited create( owner, incaption, isTopBarVisble, True );
   //
   fCustID := inCustID;
   fOrdStat := OrdStat;
   //
   OrderDetailListQuery := tMasterDataOrderListByCustID.Create( masterData, inCustID );
   //
   dataListGrid.Init( OrderDetailListQuery, 'ORGNAME');
   gridDataSource.DataSet := OrderDetailListQuery;
   gridDataSource.OnDataChange := HandleQueryUpdate;
   //
   dataListGrid.Clear;
   //
   DataListGrid.Add(OrderDetailListQuery.FieldByName('ONUM'), 'ORDER #', 60, clBlack, [fsBold], tarightJustify);
   DataListGrid.Add(OrderDetailListQuery.FieldByName('ODATE'), 'DATE', 90, clTeal, [], taRightJustify);
   DataListGrid.Add(OrderDetailListQuery.FieldByName('OTYPE'), 'OTYPE', 60, clBlack, [], taLeftJustify);
   DataListGrid.Add(OrderDetailListQuery.FieldByName('CYCLE'), 'CYCLE', 60, clBlack, [fsBold], taRightJustify);
   DataListGrid.Add(OrderDetailListQuery.FieldByName('ITEMS'), 'ITEMS', 60, clBlack, [], taRightJustify);
   DataListGrid.Add(OrderDetailListQuery.FieldByName('TOTAL'), 'TOTAL', 60, clRed, [], taRightJustify);
   DataListGrid.Add(OrderDetailListQuery.FieldByName('PAID'), 'PAID', 60, clBlue, [], taRightJustify);
   DataListGrid.Add(OrderDetailListQuery.FieldByName('DISPSTATUS'), 'STATUS', 75, $00000040, [fsBold], taLeftJustify);
   //
   dataListGrid.OnDblClick := HandleDoubleClick;
   //
   dbNavTool.Init( OrderDetailListQuery );
   //
   actionList.OnUpdate := HandleActionListUpdate;
   actionList.onActionEvent := HandleActionExecute;
   //
   OPTION_DOCK.Visible := false;
   //
   UpdateQuery();
	StartUpForm();
   StatBarUpdate();
end;

//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//

procedure tOrderSelectOrderByCustIDForm.StartUpForm;
begin
   //
   BASE_FORM_CAPTION_LABEL.Caption := 'Select Open Orders For ' + Customer_GetCustomerNameByCustID( fCustID );
   //
   UpdateQuery();
end;

//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//

function tOrderSelectOrderByCustIDForm.fOrderID: string;
begin
   result := OrderDetailListQuery.FieldByName('ID').AsString;
end;

//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//

procedure tOrderSelectOrderByCustIDForm.HandleActionExecute(sender: tObject; actionID: integer);
begin
   case actionID of
      CMD_SELECT_OK :
      begin
         fFormEvent := mrOk;
         Close();
      end;
      CMD_SELECT_CANCEL :
      begin
         fFormEvent := mrCancel;
         Close();
      end;
   end;
end;

//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//

procedure tOrderSelectOrderByCustIDForm.HandleActionListUpdate(Action: TBasicAction; var Handled: Boolean);
begin
	handled := true;
   with Action as tAction do
      case tag of
         CMD_SELECT_OK : enabled := ( OrderDetailListQuery.RecordCount <> 0);
      end;
end;

//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//

procedure tOrderSelectOrderByCustIDForm.HandleDoubleClick(sender: tObject);
begin
   fFormEvent := mrOk;
   Close();
end;

//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//

procedure tOrderSelectOrderByCustIDForm.HandleQueryUpdate(Sender: TObject; Field: TField);
begin
	StatBarUpdate();
end;

//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//

procedure tOrderSelectOrderByCustIDForm.StatBarUpdate;
begin
	StatusBar.Panels[0].Text := IntToStr(OrderDetailListQuery.RecNo) + ' of ' + IntToStr(OrderDetailListQuery.RecordCount);
end;

//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//

procedure tOrderSelectOrderByCustIDForm.UpdateQuery;
begin
   OrderDetailListQuery.Update();
end;

//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//

end.
