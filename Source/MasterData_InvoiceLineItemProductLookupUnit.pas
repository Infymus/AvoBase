...
unit MasterData_InvoiceLineItemProductLookupUnit;
...
type
   tMasterDataInvoiceLineItemProductLookup = class(tQuery)
   private
      fSQL : string;
   ...
   public
      ...
      procedure Update( inProdNum : string; inCycleID : string ); overload;
      //
      ...
      property ProductNum : string read fProdNum write fProdNum;
      //
...
constructor tMasterDataInvoiceLineItemProductLookup.Create(inMasterData: tMasterData);
var
...
procedure tMasterDataInvoiceLineItemProductLookup.HandleCalculated(DataSet: TDataSet);
begin
...
procedure tMasterDataInvoiceLineItemProductLookup.Update;
var
   workSql : string;
begin
	self.Close();
...
procedure tMasterDataInvoiceLineItemProductLookup.Update( inProdNum : string; inCycleID : string );
var
   workSql : string;
begin
   self.Close();
   //
   workSQL := fSQL;
   workSQL := workSQL + ' WHERE NUM = ' + masterData.WrapDBID(inProdNum);
   if ( inCycleID <> '' ) then
      workSQL := workSQL + ' AND C_ID = ' + masterData.WrapDBID(inCycleID);
   //
   self.SQL.Clear();
   self.SQL.Text := workSql;
   Self.Open();
