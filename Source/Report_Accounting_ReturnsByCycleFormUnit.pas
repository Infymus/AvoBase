...
unit Report_Accounting_ReturnsByCycleFormUnit;
...
interface uses
   ...
   MasterData_ReportReturnsByCycleUnit,
...
type
   tReport_Accounting_ReturnsByCycle = class(TAvoBase_ReportBase)
      Band_Group: TQRGroup;
      QRLabel1: TQRLabel;
      db_cycle: TQRLabel;
...
constructor tReport_Accounting_ReturnsByCycle.create(Owner: tComponent );
begin
...
destructor tReport_Accounting_ReturnsByCycle.destroy;
begin
   FreeAndNil(	ReturnInvoiceObj );
   inherited destroy;
end;

procedure tReport_Accounting_ReturnsByCycle.SetOptions( inOrgID, inStartCycleID,
   inEndCycleID : string; inOptOpen, inOptClosed, inOptCancel : boolean);
var
   startCycleRec : tCycleRec;
   endCycleRec : tCycleRec;
   tempQuery: tQuery;
   fReportQuery : tMasterDataReportReturnList;
...
begin
   ...
   fReportQuery := tMasterDataReportReturnList.Create(
      masterData,
      inOrgID,
      startCycleRec.year,
      endCycleRec.year,
...
function tReport_Accounting_ReturnsByCycle.CanPrint: string;
begin
   result := ferrResult;
end;
...
procedure tReport_Accounting_ReturnsByCycle.Band_GroupBeforePrint( Sender: TQRCustomBand; var PrintBand: Boolean);
begin
   inherited;
   //
...
// DETAIL BAND
procedure tReport_Accounting_ReturnsByCycle.Band_DetailBeforePrint( Sender: TQRCustomBand; var PrintBand: Boolean );
begin
   inherited;
   //
...
// Group Totals
procedure tReport_Accounting_ReturnsByCycle.ChildBand3BeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
   inherited;
   //
...
// Report Totals
procedure tReport_Accounting_ReturnsByCycle.ChildBand1BeforePrint( Sender: TQRCustomBand; var PrintBand: Boolean);
begin
   inherited;
   //
