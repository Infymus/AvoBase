...
unit MasterData_ReportAccountingOrderAmountByCycleUnit;
...
type
   tMasterDataReportOrderAmountBreakDownByCycle = class(tQuery)
   PRIVATE
      fShowOpen : boolean;
      fShowClosed : boolean;
      fShowCancel : boolean;
...
constructor tMasterDataReportOrderAmountBreakDownByCycle.Create( inMasterData : tMasterData; inOrgID : string;
   inStartYear, inEndYear, InStartNum, InEndNum : integer;
...
begin
   ...
   //
   sqlText := 'SELECT O.ID, O.STATUS, O.O_TYPE, O.ORG_ID, O.C_ID, O.ONUM, C.ID, C.NUM, C.CYEAR, C.ORG_ID ' +
   ...
   begin
      ...
      sqlWhere := sqlWhere +  '((CYEAR = ' + IntToStr( inStartYear ) + ') AND (NUM BETWEEN ' + IntToStr( inStartNum ) + ' AND 30)' +
         ' AND (' + sqlOpt + ') AND (O.ORG_ID = ' + masterData.WrapDBID(InOrgID) + ') AND (O_TYPE=1)) ';
      //
   ...
   sqlText := sqlText + ' ORDER BY O.ONUM';
   self.SQL.Clear();
...
destructor tMasterDataReportOrderAmountBreakDownByCycle.destroy;
begin
   //
   inherited;
end;
