...
unit Report_Accounting_TaxExemptByCycleFormUnit;
...
interface uses
   ...
   MasterData_ReportAccountingTaxExemptByCycleUnit,
   //
   db,
   bde,
   windows,
...
type
   tReport_Accounting_TaxExemptByCycle = class(TAvoBase_ReportBase)
      QRLabel1: TQRLabel;
      QRLabel2: TQRLabel;
      QRLabel3: TQRLabel;
      QRLabel4: TQRLabel;
...
constructor TReport_Accounting_TaxExemptByCycle.create(Owner: tComponent );
begin
...
destructor TReport_Accounting_TaxExemptByCycle.destroy;
begin
...
procedure TReport_Accounting_TaxExemptByCycle.SetOptions( inOrgID, inStartCycleID, inEndCycleID : string );
var
...
function TReport_Accounting_TaxExemptByCycle.CanPrint: string;
begin
   result := ferrResult;
end;
...
procedure TReport_Accounting_TaxExemptByCycle.Band_DetailBeforePrint( Sender: TQRCustomBand; var PrintBand: Boolean);
begin
   inherited;
   //
   db_onum.Caption := fReportQuery.FieldByName('ONUM').AsString;
   db_odate.Caption := fReportQuery.FieldByName('ODATE').AsString;
   db_cycle.Caption := fReportQuery.FieldByName('CYCLENAME').AsString;
   db_dispstatus.Caption := fReportQuery.FieldByName('STATUS').AsString;
   db_tottax.Caption := FormatCurrency( fReportQuery.FieldByName('ORDTAX').AsCurrency );
   db_TAXEXID.Caption := fReportQuery.FieldByName('TAXEXID').AsString;

{
   ...
   //
   fTotal := fTotal + fReportQuery.FieldByName('ORDTAX').AsCurrency;
end;

procedure TReport_Accounting_TaxExemptByCycle.ChildBand1BeforePrint( Sender: TQRCustomBand; var PrintBand: Boolean);
begin
   inherited;
   //
