...
unit MasterData_ReportAccountingTaxesCollectedByCycleUnit;
...
begin
   ...
   begin
      ...
      sqlWhere := sqlWhere +  '((CYEAR = ' + IntToStr( inStartYear ) + ') AND (NUM BETWEEN ' + IntToStr( inStartNum ) + ' AND 30)' +
         ' AND (' + sqlOpt + ') AND (O.ORG_ID = ' + masterData.WrapDBID(InOrgID) + ') AND (O_TYPE=' + sqlOrdType + ')) ';
      //
   ...
   masterData.QueryAddCalculatedField( self, 'FEETAX', 1, ftCurrency);
   masterData.QueryAddCalculatedField( self, 'ORDTAX', 1, ftCurrency);
   masterData.QueryAddCalculatedField( self, 'TOTTAX', 1, ftCurrency);
   //
...
procedure tMasterDataReportAccountingTaxCollectedByCycle.HandleCalculated(DataSet: TDataSet);
begin
   case Self.FieldByname('O_TYPE').AsInteger of
      ...
      begin
         ...
         DataSet.FieldByName('FEETAX').AsCurrency := InvoiceObj.Amount_FeeItemTax;
         DataSet.FieldByName('ORDTAX').AsCurrency := InvoiceObj.Amount_LineItemTax;
         DataSet.FieldByName('TOTTAX').AsCurrency := InvoiceObj.Amount_TotalTax;
